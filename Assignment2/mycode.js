var vg_1 = "gdp.vg.json";
vegaEmbed("#bubblechart", vg_1).then(function (result) {
}).catch(console.error);
var vg_2 = "waste.vg.json";
vegaEmbed("#map1", vg_2).then(function (result) {
}).catch(console.error);
var vg_3 = "waste_generated_treated.vg.json";
vegaEmbed("#bar1", vg_3).then(function (result) {
}).catch(console.error);
var vg_4 = "plastic_waste.vg.json";
vegaEmbed("#map2", vg_4).then(function (result) {
}).catch(console.error);
var vg_5 = "plastic_disposal.vg.json";
vegaEmbed("#bar2", vg_5).then(function (result) {
}).catch(console.error);
